...
const LocalThemedBox = () => {
    const [theme, ss] = useContext(ThemeContext)
    ...
    return(
        <div className={"bg-"+localTheme} style={{width:'200px',height:'200px',border:'2px solid green'}} id="local-themed-box">
            ...
            <button className={"btn btn-"+localTheme+" txt-"+localTheme} id="local-theme-toggler" onClick={changeLocalTheme}>Toggle local theme to {localTheme==='dark'?'light':'dark'}</button>
        </div>
    )
}
